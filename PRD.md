...
本项目旨在开发一个高度自动化、基于大型语言模型（LLM）的肝脏代谢模拟工具。该工具的核心目标是利用 LLM 的知识库和推理能力，自动生成、细化、格式化肝脏代谢的输入/输出数据和参数，并自动构建、运行、调优基于生产者-消费者模型的模拟代码，以实现对真实肝脏代谢流的精确模拟。
...
**R1.1 自动生成模块层次结构**
  ...
  * **需求：** LLM 必须能够从高粒度（如：碳水代谢）自动向下细化到中粒度（如：糖异生、糖原合成），并生成每个细化粒度（模块函数）的完整输入和输出描述。
  * **输入：** 肝脏主要功能模块名称列表（例如：碳水代谢、脂代谢、解毒模块等）。
  * **输出：** **多层级**的代谢模块清单，包含每个模块的：
      * 模块名称/功能。
      * 上层/下层模块依赖关系。
      * 完整的代谢物输入列表。
      * 完整的代谢物输出列表。
...
**R2.1 自动构建生产者-消费者模型代码**
  ...
  * **需求：** LLM 必须基于 R1.3 生成的 JSON 元数据，自动构建模拟肝脏代谢流的生产者-消费者模型代码。
  * **模型映射：**
      * 每个细化模块（如“糖异生”）映射为一个函数。
      * 代谢物映射为共享资源池或队列。
...
**R2.2 集成参数与速率方程**
  ...
  * **需求：** 自动将 R1.4 中生成的参数集成到 R2.1 的代码中，并使用参数来计算各模块的代谢速率（流量）。
  * **速率函数：** LLM 需自动选择或生成简化的速率方程（如一级反应速率或 Michaelis-Menten 简化形式）。

**R2.3 自动运行模拟**

  * **需求：** LLM 或其包装程序能够自动执行生成的模拟代码，并在预设时间步长内记录所有关键代谢物的浓度变化（流量）。
...
**R3.2 自动调整代码和参数**
  ...
  * **需求：** 基于 R3.1 的偏差分析，LLM 必须能够自动诊断代码或参数的问题，并执行以下操作：
      * **参数微调：** 根据偏差程度，对 R1.4 中生成的参数值（如 $V_{max}$、激素敏感性）进行迭代式微调。
      * **代码调整（高阶需求）：** 在必要时，LLM 需自动修改 R2.1 中的代码逻辑或速率方程，以更好地匹配真实代谢流。
...
| $\text{AC} 4$ | 生产者-消费者模型 | 代码结构清晰体现生产者-消费者模型，证明并发/并行代谢流被合理模拟。 |
